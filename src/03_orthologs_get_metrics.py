...
logistic regression model for the non-redundant gene set collections when 
including orthologs across species.
'''

parser = argparse.ArgumentParser()
parser.add_argument('-g','--geneset_info',
                    default = 'MethodNum-0__GO_hs_20_100_0.5_0.5',
                    type = str,
                    help = 'information needed to make genesets')
parser.add_argument('-n','--network_info',
                    default = 'hs_mm__BioGRID_raw__eggnog_direct_SummedDegree-0.50__Pecanpy_500_SparseOTF_0.1_0.0001_120_8_10_weighted_directed.emb',
                    type = str,
...
# find the orthologs
GSC_species = geneset_info.split('__')[-1].split('_')[1]
net_species = network_info.split('__')[0].split('_')
network_type= network_info.split('__')[1]

print(GSC_species,net_species)
for idx, akey in enumerate(label_dict):
    for aspecies in net_species:
        if aspecies == GSC_species:
            continue
        else:
            if aspecies > GSC_species:
                s1 = GSC_species; s2 = aspecies
            else:
                s2 = GSC_species; s1 = aspecies
            fp_main_net = '../data/edgelists/connections/'
            fp_end_con = '%s_%s__%s__direct__AllOnes'%(s1,s2,network_type)
            df_con = pd.read_csv(fp_main_net + '%s.edgelist'%fp_end_con,
                                 sep='\t',header=None,names=[s1,s2,'Weight'],dtype=str)
            orthos_to_add_full = []
            for agene in label_dict[akey]['Train-Positive']:
                orthos_to_add = df_con[df_con[GSC_species]==agene][aspecies].tolist()
                orthos_to_add_full = list(set(orthos_to_add_full + orthos_to_add))
            label_dict[akey]['Train-Positive'] = label_dict[akey]['Train-Positive'] + orthos_to_add_full
...
for idx, akey in enumerate(label_dict):
    ...
    results_tmp.append([geneset_info,
                        net_combo[1],net_combo[0]+'_orthos',net_combo[2],net_combo[3], model_info,
...
save_dir = '../results/ortholog_metrics/'

...
'''
This script will take the supplied parameters and train and evaluate a
logistic regression model for the matched gene set collections.
'''
...
tic3 = time.time()
print('Doing ML part')
for label_dict, method in ((label_dict_sp0,'Trn-%s-Tst-%s'%(sp0,sp0)),
                           (label_dict_sp1,'Trn-%s-Tst-%s'%(sp1,sp1)),
                           (label_dict_Bothsp0,'Trn-%s_%s-Tst-%s'%(sp0,sp1,sp0)),
                           (label_dict_Bothsp1,'Trn-%s_%s-Tst-%s'%(sp0,sp1,sp1))):
    results_tmp = []
    ...
    save_dir = '../results/bothterms_metrics/'
    df_results.to_csv(save_dir + 'MetricResults--BothTerms--%s--%s--%s--%s--%s.tsv'%(task,method,args.geneset_info,args.network_info,args.model_info),
